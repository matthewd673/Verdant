...
public static class Renderer
{
    ...
    // The number of EntityDrawCalls made so far this frame.
    // NOTE: Enabling VisualizeBounds requires additional draw calls but will not be counted here.
    public static int EntityDrawCalls { get; private set; }
    ...
    public static void Render(SpriteBatch spriteBatch, Scene scene)
    {
        ...
        if (VisualizeBodies)
        {
            foreach (Entity e in entities)
            {
                if (e.IsType<Physics.PhysicsEntity>())
