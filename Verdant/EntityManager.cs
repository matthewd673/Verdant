...
public class EntityManager
{
    ...
    // The number of Entities updated so far this tick.
    public int EntityUpdateCount { get; protected set; }
    // The number of PhysicsEntities updated so far this physics loop (within the this tick).
    ...
    {
        foreach (EntityList l in table.GetCells())
        {
            Verdant.Debugging.Log.WriteLine(l.GetEntities().Count);
            foreach (Entity e in l.GetEntities())
            {
                if (e.IsType<TEntity>())
                {
                    yield return (TEntity)e;
                }
            }
        }
    }
    ...
    public IEnumerable<TEntity> GetEntitiesInBounds<TEntity>(float x,
                                                             float y,
                                                             float width,
                                                             float height
    ...
    public IEnumerable<Entity> GetEntitiesInBounds(float x,
                                                   float y,
                                                   float width,
                                                   float height)
    {
    ...
    public List<TPhysicsEntity> CheckRectCollisions<TPhysicsEntity>(float x, float y, int width, int height, bool includeTriggers = false, bool includeSolids = true, List<TPhysicsEntity> ignoreList = null) where TPhysicsEntity : PhysicsEntity
    {
        ...
        IEnumerable<TPhysicsEntity> searchList = GetEntitiesInBounds<TPhysicsEntity>(x, y, width, height);
    ...
    private void PhysicsLoop(List<PhysicsEntity> updateList)
    {
        ...
        for (int i = 0; i < updateList.Count; i++)
        {
            ...
            IEnumerable<EntityList> range = table.GetCellRange(searchKey.X - 1, searchKey.Y - 1, searchKey.X + 1, searchKey.Y + 1);
            foreach (EntityList r in range)
            {
                List<PhysicsEntity> cellPhysicsEntities = r.GetPhysicsEntities();
                foreach (PhysicsEntity b in cellPhysicsEntities)
                {
    ...
    /// </summary>
    /// <param name="collection">An enumerable of Entities to update.</param>
    protected virtual void UpdateCollection(IEnumerable<Entity> collection)
    {
        EntityUpdateCount = 0;
